...
vector<int> getImageData(const vector<uint8_t>& input, int offset, const int& size){
	...
	cout << binaryInput.size() / 8 << endl;

	while(!binaryInput.empty()){
		counter++;
		//cout << "CODE SIZE: " << codeSize << " Binary SIZE: " << binaryInput.size() << endl;
		if(counter == 1107){
			print(binaryInput);
		}
		pW = cW;
		for(int i = 0; i < codeSize; i++){
			myfile << binaryInput[i];
		}
		cW = binToHex(extract(binaryInput, codeSize));
		
		myfile << "STEP " << counter << endl;
		myfile << "--------------------------------------------";
		myfile << "\npW: " << pW << "\t cW: " << cW << endl << endl;
		//myfile << "Code Table: \n------------------------\n";
		/*for(int i = 0; i < codeTable.size(); i++){
			myfile << "Position " << i << " | ";
			myfile << codeTable[i] << endl;
		}*/
		myfile << "Code Table Size: " << codeTable.size() << endl;
		myfile << "Code Stream: \n";
		...
		if(cW > codeTable.size() - 1){ //aka cW isn't in the dictionary
			if(pW >= codeTable.size()) {
				cout << "BEEP " << binaryInput[counter] << ' ' << counter << endl;
				break;
			}
			ss << codeTable[pW];
			p = ss.str();
			...
			codeStream.push_back(cW);
			indexStream.push_back(codeTable[cW]);
			if(cW >= codeTable.size() || pW >= codeTable.size()){
				cout << "MEGABEEP " << binaryInput[counter] << ' ' << counter;
				break;
			}
			ss << codeTable[pW];
			p = ss.str();
